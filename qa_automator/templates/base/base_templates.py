...
MENSAGEM_INICIAL: str = """
    ✳️ BEM VINDO AO SISTEMA DE CRIAÇÃO DE TESTES AUTOMÁTICO ✳️
...
def create_base_structure(project_path: Path, project_name: str, frameworks: list):
    ...
    (project_path / "__init__.py").write_text(get_init_py())
    
    # Criar agent.py
    (project_path / "agent.py").write_text(get_agent_py())
    
    # Criar prompt.py
    (project_path / "prompt.py").write_text(get_prompt_py())
    
    # Criar tools.py
    (project_path / "tools.py").write_text(get_tools_py())
    
    # Criar main.py
    (project_path / "main.py").write_text(get_main_py(project_name))
    
    # Criar diretório prompts
    prompts_path = project_path / "prompts"
    prompts_path.mkdir(exist_ok=True)
    (prompts_path / "__init__.py").write_text(get_init_py())
    
    # Criar prmt_main.py
    framework_names = [fw["name"] for fw in frameworks]
    (prompts_path / "prmt_main.py").write_text(get_prmt_main_py(framework_names))
